...
By default, this wrapper loads `https://lalgeo.com/lalgeosurvey.html` until a MapKit token env var is present. When `MAPKIT_TOKEN` or `NEXT_PUBLIC_MAPKIT_TOKEN` is set, it loads `/legacy/lalgeosurvey`, a Next route that serves `public/legacy/lalgeosurvey.html` with that token injected.
