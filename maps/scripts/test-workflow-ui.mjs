...
assert.match(source, /id="workspaceCreateModal"[\s\S]*?<h3>New Project<\/h3>[\s\S]*?<label for="createProjectName">Name<\/label>[\s\S]*?<label for="createLayerName">First Layer<\/label>[\s\S]*?<summary>Options<\/summary>/, "New Project should show only essential fields before optional settings.");
assert.match(source, /id="layerImportModal"[\s\S]*?<h3 id="layerImportTitle">Add a layer<\/h3>[\s\S]*?id="layerImportPickerBtn"[^>]*class="layer-import-choice"[\s\S]*?<strong>Choose files<\/strong>[\s\S]*?<summary>Use a public URL<\/summary>/, "Layer import should lead with file selection and progressively disclose URL help.");
...
assert.match(source, /<h3>Open a project<\/h3>[\s\S]*?<summary>Share &amp; Export<\/summary>[\s\S]*?id="downloadProjectBtn"[^>]*>Export Project<\/button>[\s\S]*?<summary>Dropbox<\/summary>[\s\S]*?<summary>Project Files<\/summary>/, "Project management should group secondary tools into short disclosures.");
