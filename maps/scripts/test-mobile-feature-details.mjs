...
const checks = [
  ...
  ["the Properties action can explicitly open existing feature details", /function openFeaturePropertiesByRow\(rowIndex, returnFocus = document\.activeElement\)[\s\S]*?previewAnnotationByRow\(rowIndex, \{ preserveSelectionView: true \}\);[\s\S]*?renderFeatureDrawer\(\{ forceOpen: true, focusOnOpen: true, returnFocus, returnAction: "properties" \}\);/],
  ["the Edit action opens editable feature details for the selected layer", /function openFeatureEditorByRow\(rowIndex, returnFocus = document\.activeElement\)[\s\S]*?const layerId = activeSurveyAnnotation\?\.surveyPoint\?\.layerId \|\| activeLayerId;[\s\S]*?setEditSessionActive\(true, \{ layerId \}\);[\s\S]*?if \(!editSessionActive\) return;[\s\S]*?renderFeatureDrawer\(\{ forceOpen: true, focusOnOpen: true, returnFocus, returnAction: "edit" \}\);/],
