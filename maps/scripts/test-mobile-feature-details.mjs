...
const checks = [
  ...
  ["the Edit action opens editable feature details", /function openFeatureEditorByRow\(rowIndex\)[\s\S]*?setEditSessionActive\(true\);[\s\S]*?if \(!editSessionActive\) return;[\s\S]*?renderFeatureDrawer\(\{ forceOpen: true \}\);/],
