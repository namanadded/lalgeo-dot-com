...
const checks = [
  ...
  ["existing mobile selections do not automatically open feature details", /function renderFeatureDrawer\(\{ newFeature = false \} = \{\}\)[\s\S]*?matchMedia\("\(max-width: 600px\)"\)\.matches[\s\S]*?!newFeature[\s\S]*?!featureDrawer\.classList\.contains\("open"\)[\s\S]*?closeFeatureDrawer\(\);[\s\S]*?return;/],
